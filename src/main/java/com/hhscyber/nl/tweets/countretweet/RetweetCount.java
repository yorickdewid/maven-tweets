...
public class RetweetCount {
    ...
    public static void main(String[] args) throws IOException, Exception {
        ...
        Job job = new Job(conf, "CountRetweet");
        job.setJarByClass(RetweetCount.class);
        String stop = "631415321467965440"; //10 tweets
        Scan scan = new Scan();
        //scan.setStopRow(stop.getBytes());

        TableMapReduceUtil.initTableMapperJob("hhscyber:tweets_filtered", scan, RetweetCountMapper.class, ImmutableBytesWritable.class, Result.class, job);
        job.setNumReduceTasks(1);

        TableMapReduceUtil.initTableReducerJob("hhscyber:tweets_location_test", RetweetCountReducer.class, job); // if disabled no output folder specfied exception
